...
float Smiley(vec2 uv, vec2 pos, float size) { 
    ...
    mask -= Circle(uv, vec2(-size * 0.3, size * 0.3), size * 0.2, 0.01);
    mask -= Circle(uv, vec2(size * 0.3, size * 0.3), size * 0.2, 0.01);
    
    float mouth = Circle(uv, pos, size * 0.8, 0.01);
    mouth -= Circle(uv, vec2(pos.x, pos.y + size * 0.1), size * 0.8, 0.01);
...
void main() { 
    ...
    float smileMask = Smiley(smileUv, vec2(0.0), sin(u_time * 3.0) * 0.1 + 0.3);
    vec3 c = vec3(1.0, 1.0, 0.0) * smileMask;
    
    float rectMask = Rect(uv, vec2(-0.4, 0.0), vec2(0.2, 0.3), (sin(u_time * 5.0) * 0.4 + 0.5) * 0.05);
    c += vec3(1.0, 0.0, 1.0) * rectMask;
